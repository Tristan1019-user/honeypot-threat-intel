...
class PaginationCursor(BaseModel):
    limit: int = Field(..., description="Requested page size")
    cursor: Optional[str] = Field(None, description="Current cursor position (monotonic rowid)")
...
class IndicatorRecord(BaseModel):
    ...
    collection_window: CollectionWindow = Field(default_factory=CollectionWindow, description="Observation time window")
...
class IntegrityResponse(BaseModel):
    stix_bundle_sha256: str = Field(..., description="SHA-256 hash of the full STIX bundle (sorted keys)")
    stix_object_count: int = Field(..., description="Number of STIX objects in the bundle")
    total_sessions: int = 0
    total_indicators: int = 0
    last_update: Optional[str] = None
