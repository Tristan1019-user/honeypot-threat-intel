...
# In-memory cache for expensive full STIX bundle assembly.
# WARNING: per-worker cache. Under --workers=N each worker builds its own bundle
# independently, consuming N× memory. Keep uvicorn at --workers=1 (compose default)
# or move to a shared cache (Redis) for multi-worker deployments.
STIX_FULL_CACHE: dict[str, Any] = {"etag": None, "payload": None, "ts": 0.0}
STIX_FULL_CACHE_TTL_SECONDS = 300
# Cap the full STIX bundle to the N most-recent sessions to bound RSS.
# At ~3 KB/session bundle the merge tops out at ~225 MB for 75k sessions.
# Sessions beyond this cap are still reachable via /api/v1/feed (paginated JSON/CSV).
STIX_MAX_SESSIONS = 75_000
# Prevents concurrent cold-cache builds under simultaneous requests.
# Without a lock, two requests arriving while the cache is cold each start
# the full 75k-session loop independently, doubling RAM usage.
_STIX_CACHE_LOCK = asyncio.Lock()
...
def parse_since(since: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        # OSError/OverflowError: fromtimestamp raises these for out-of-range values
        # (e.g. float("inf") or 1e309 pass the > 0 check but overflow the C time_t).
        pass
...
async def _ensure_stix_cache() -> None:
    """Populate STIX_FULL_CACHE if absent or expired.

    Called by both feed_stix() and feed_stix_gz() so neither endpoint has to
    route through the other to warm the cache.

    _STIX_CACHE_LOCK prevents concurrent cold-cache builds: the first request
    acquires the lock and builds; subsequent concurrent requests wait, then
    find the cache already warm and return immediately.

    NOTE: full bundle merge is memory-heavy (O(sessions × bundle size)).
    STIX_MAX_SESSIONS caps the build at 75k sessions to bound RSS.
    Streaming/chunked export is deferred as future work; keep --workers=1
    in compose.yml or move STIX_FULL_CACHE to Redis for multi-worker deployments.
    """
    # Fast path outside the lock (avoid lock contention on every warm-cache hit).
    now = time.time()
    if STIX_FULL_CACHE.get("payload") is not None and (now - float(STIX_FULL_CACHE.get("ts", 0.0))) < STIX_FULL_CACHE_TTL_SECONDS:
        return

    async with _STIX_CACHE_LOCK:
        # Re-check inside the lock: a concurrent request may have just built the cache.
        now = time.time()
        if STIX_FULL_CACHE.get("payload") is not None and (now - float(STIX_FULL_CACHE.get("ts", 0.0))) < STIX_FULL_CACHE_TTL_SECONDS:
            return

        # Memory-optimized STIX merge: dedupe objects on the fly instead of storing all bundles.
        objects_by_id: dict[str, Any] = {
            str(PRODUCER_IDENTITY["id"]): PRODUCER_IDENTITY,
            str(TLP_CLEAR_MARKING["id"]): TLP_CLEAR_MARKING,
        }
        anonymous_objects: list[dict] = []

        page_size = 2000
        offset = 0
        total_fetched = 0
        last_page_was_full = False
        while total_fetched < STIX_MAX_SESSIONS:
            fetch_size = min(page_size, STIX_MAX_SESSIONS - total_fetched)
            sessions = await db.query_sessions(limit=fetch_size, offset=offset)
            if not sessions:
                break
            ...
            for s in sessions:
                ...
                try:
                    ...
                    for obj in bundle.get("objects", []):
                        ...
                        if oid:
                            objects_by_id.setdefault(oid, obj)
                        elif isinstance(obj, dict):
                            anonymous_objects.append(obj)
                except (json.JSONDecodeError, TypeError, AttributeError):
                    continue

            total_fetched += len(sessions)
            last_page_was_full = len(sessions) == fetch_size
            if not last_page_was_full:
                break
            offset += fetch_size

        # Only warn when we actually hit the cap mid-dataset (more rows exist).
        # If total_fetched == STIX_MAX_SESSIONS but the last page was partial,
        # the dataset is exactly that size — nothing was excluded.
        if total_fetched >= STIX_MAX_SESSIONS and last_page_was_full:
            logger.warning(
                "STIX bundle capped at %d sessions (STIX_MAX_SESSIONS). "
                "Older sessions are excluded from /feed/stix but remain accessible via /feed (paginated).",
                STIX_MAX_SESSIONS,
            )
        ...
        merged = {
            ...
            "objects": list(objects_by_id.values()) + anonymous_objects,
        }

        result = scrub_dict(merged)
        etag = _etag(result)
        STIX_FULL_CACHE.update({"etag": etag, "payload": result, "ts": now})
...
async def feed_stix(request: Request):
    ...
    await _ensure_stix_cache()
    result = STIX_FULL_CACHE["payload"]
    etag = STIX_FULL_CACHE["etag"]
    cached = _check_etag(request, etag)
    if cached:
        return cached
    return AppJSONResponse(content=result, headers=_cache_headers(etag, max_age=600))


@app.get("/api/v1/feed/stix.gz", tags=["Feed"])
@limiter.limit("30/minute")
async def feed_stix_gz(request: Request):
    """Gzipped STIX 2.1 bundle export for efficient downloads.

    Uses the same in-memory STIX bundle cache as /api/v1/feed/stix (5-minute TTL)
    but computes its own gzip ETag — so 304 decisions are based on the gzip
    content hash, not the JSON payload ETag from feed_stix().
    """
    # Ensure the STIX bundle cache is warm (build if stale).
    # _ensure_stix_cache() is the shared builder — both this endpoint and
    # feed_stix() call it directly.  Each endpoint then computes its own ETag
    # (JSON payload ETag vs. gzip content ETag) independently.
    await _ensure_stix_cache()

    payload_obj = STIX_FULL_CACHE.get("payload")
    if payload_obj is None:
        raise HTTPException(status_code=503, detail="STIX bundle cache unavailable")

    payload_bytes = json.dumps(payload_obj).encode()
    gz = gzip.compress(payload_bytes, compresslevel=6)
    etag = f'"{hashlib.sha256(gz).hexdigest()[:32]}"'
    cached = _check_etag(request, etag)
    if cached:
        return cached
    ...
    return Response(
        ...
        headers={
            **_cache_headers(etag, max_age=300),
            "Content-Disposition": 'attachment; filename="stix_full.json.gz"',
        },
    )
