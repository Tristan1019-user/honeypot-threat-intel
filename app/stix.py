...
def _deterministic_uuid(namespace: str, *parts) -> str:
...
def _stix_id(stix_type: str, *parts) -> str:
...
def _threat_level_to_confidence(threat_level: str) -> int:
    """Map threat level to STIX confidence score."""
    return {"critical": 95, "high": 85, "medium": 65, "low": 40}.get(threat_level, 50)


def session_to_stix_bundle(session: dict) -> dict:
    """
...
def merge_stix_bundles(bundles: list[dict]) -> dict:
